...
def google_search(query: str, num_results: int = 2, max_chars: int = 500) -> list:  # type: ignore[type-arg]
    ...
    api_key = "AIzaSyDJkjepzNjacHfj4Zf645qowkb7WNDwKgQ"
    #api_key = os.getenv("GOOGLE_API_KEY")
    search_engine_id = "07253fe460af9419f"
...
def analyze_stock(ticker: str) -> dict:  # type: ignore[type-arg]
    ...
    plot_file_path = f"graphic/{ticker}_stockprice.png"
    plt.savefig(plot_file_path)
    print(f"Plot saved as {plot_file_path}")
    result["plot_file_path"] = plot_file_path

    return result
#asyncio.run(Console(group_chat.run_stream(task="Write a unique, Haiku about the weather in Paris")))
...
search_agent = AssistantAgent(
    ...
    model_client=OpenAIChatCompletionClient(model="gpt-4o-mini", api_key="sk-u4V8dGD5v1fSLgtYP-j7Jwz6wdBRu7XHG52EpWoNFxT3BlbkFJ0gkbtuVQpj5b_Pkh4pT80La_aofoLYCZvvvZQGWAEA"),
...
if __name__ == "__main__":
    asyncio.run(Console(team.run_stream(task="Write a financial report on Endava")))